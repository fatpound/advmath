...
public:
    CubeFoldedWrapped(float size)
    {
        ...
        texCoords.emplace_back(1.0f, 3.0f);

        vertices.emplace_back( side, -side,  side); // 5
        texCoords.emplace_back(0.0f, 3.0f);

        vertices.emplace_back(-side,  side,  side); // 6
        texCoords.emplace_back(1.0f, 2.0f);

        vertices.emplace_back( side,  side,  side); // 7
        texCoords.emplace_back(0.0f, 2.0f);

        vertices.emplace_back(-side, -side, -side); // 8
        texCoords.emplace_back(1.0f, 4.0f);

        vertices.emplace_back( side, -side, -side); // 9
        texCoords.emplace_back(0.0f, 4.0f);

        vertices.emplace_back(-side, -side, -side); // 10
        texCoords.emplace_back(2.0f, 1.0f);

        vertices.emplace_back(-side, -side,  side); // 11
        texCoords.emplace_back(2.0f, 2.0f);
...
public:
    ...
    {
        throw std::runtime_error("Incomplete function CubeFoldedWrapped::GetLines!");
        //return{
        //	vertices,{
        //		0,1,  1,3,  3,2,  2,0,
        //		0,4,  1,5,	3,7,  2,6,
